...
int main(int argc, char* argv[]) {
    try {
        ...
        auto human1Entity = std::make_shared<StaticEntity>(mesh_triangles, Pose({0, 0, 0}, 0, 0, 45));
        ...
        auto myLidar = std::make_shared<LidarEntity>(ousterModel, 2, 360, lidarPose);

        world.addLidar(myLidar);
        world.build();
