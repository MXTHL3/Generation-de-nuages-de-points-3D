    ...
    if(j.contains("lidar_entities")){
        for(const auto& item : j["lidar_entities"]){
            ...
            if(auto mechanical_lidar_config = std::dynamic_pointer_cast<MechanicalLidarConfig>(shared_lidar_config)){
                ...
                SIM_DEBUG("{}", shared_lidar_config->to_string());
                std::unique_ptr<LidarEntity> lidar_ent = std::make_unique<FlashLidarEntity>(flash_lidar_config, pose);
                lidars_ent.push_back(std::move(lidar_ent));
            }else if(auto mirrored_lidar_config = std::dynamic_pointer_cast<MirroredLidarConfig>(shared_lidar_config)){
                std::unique_ptr<LidarEntity> lidar_ent = std::make_unique<MirroredLidarEntity>(mirrored_lidar_config, pose);
                lidars_ent.push_back(std::move(lidar_ent));
                SIM_DEBUG("{}", shared_lidar_config->to_string());
            }
        }
